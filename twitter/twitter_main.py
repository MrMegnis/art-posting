...
def scroll_and_collect_imgs(driver, with_meta=False, min_scrolls=100, max_scrolls=150, max_images=10_000, min_pause=1.5,
                            max_pause=3.0):
            ...
            try:
                ...
                if with_meta:
                    ...
                    try:
                        ...
                        tweet_data['author'] = ""
                    try:
                        tweet_link = tw.find_element(By.XPATH, ".//a[contains(@href, '/status/')]").get_attribute(
                            "href")
...
def main(
        ...
        out_dir="raiden_images",
        override_file=True
):
    url, _ = build_twitter_search_query(
        keywords=keywords or ['Raiden OR "Raiden Shogun" OR 雷電将軍', 'Genshin Impact'],
        exact=exact,
        filter_images=filter_images if filter_images is not None else True,
        since=since,
        until=until,
        lang=lang,
        mode=mode or "live"
    ...
    try:
        ...
        if cookies_valid(driver, url):
            ...
            img_urls, meta_data = scroll_and_collect_imgs(driver, with_meta=with_meta, min_scrolls=min_scrolls,
                                                          max_scrolls=max_scrolls, max_images=max_images,
                                                          min_pause=min_pause, max_pause=max_pause)
            with open(results_file, "w" if override_file else "a", encoding="utf-8") as f:
                for url in img_urls:
                    f.write(url + "\n")
            if with_meta:
                if not override_file and os.path.exists(meta_file):
            ...
            if download and img_urls:
                download_images(img_urls, out_dir)
            driver.quit()
            return
        driver.quit()
    except Exception as e:
...
def batch_main(days=7, **kwargs):
    today = datetime.today()
    for i in range(days):
        since = (today - timedelta(days=i + 1)).strftime("%Y-%m-%d")
        until = (today - timedelta(days=i)).strftime("%Y-%m-%d")
        print(f"Парсинг за {since} — {until}")
        main(since=since, until=until, override_file=False, **kwargs)
        time.sleep(random.uniform(1, 5))
        if random.random() < 0.2:
            print("⏸️ Делаем длинную паузу между днями...")
...
if __name__ == "__main__":
    ...
    batch_main(
        ...
        with_meta=True,
        max_images=300,
        results_file=f"raiden_images_days.txt"
    )
